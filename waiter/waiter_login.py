...
def lambda_handler(event, context):
    ...
    if data.get('password') and data.get('email'):
        ...
        waiter.get_login_info(event)
        login_status = waiter.login(email, password)

        event_length = len(data)
        if waiter.login_user == True:

            if event_length == 3:
                login_info = waiter.get_login_info(event)
